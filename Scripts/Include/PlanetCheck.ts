...
# SUB:       PlanetCheck
# Passed:    $PlanetCheckSub - Planet check sub
#            $IgnoreList - List of planet IDs to ignore, separated by spaces
# Triggered: Sector command prompt
# Returned:  $found - Equals planet number if found what was looking for
#            Sub will remain on the planet if anything is found.  Otherwise will exit
#            to sector.
...
:PlanetCheck
  ...
  :sub_Check
    ...
    if ($ignore = 0)
      gosub $planetCheckSub
      
      if ($found = 1)
        setVar $found $Check_Planet
      end
    end
