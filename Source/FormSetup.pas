...
begin
  ...
  cbUseAuthProxy.Checked := TWXAuth.UseAuthProxy;
  cbAuthenticate.Checked := TWXAuth.Authenticate;
  ...
  // don't call UserChange event
  tbUser.OnChange := nil;
  tbRegKey.OnChange := nil;
  tbProxyAddress.OnChange := nil;
  tbProxyPort.OnChange := nil;
  cbUseAuthProxyClick(Self);
  tbUser.Text := TWXAuth.UserName;
  tbRegKey.Text := TWXAuth.UserReg;
  tbProxyAddress.Text := TWXAuth.ProxyAddress;
  tbProxyPort.Text := IntToStr(TWXAuth.ProxyPort);
  tbUser.OnChange := tbUserChange;
  tbRegKey.OnChange := tbUserChange;
  tbProxyAddress.OnChange := tbUserChange;
  tbProxyPort.OnChange := tbUserChange;
...
begin
  ...
  TWXAuth.UseAuthProxy := cbUseAuthProxy.Checked;
  TWXAuth.ProxyAddress := tbProxyAddress.Text;

  try
    TWXAuth.ProxyPort := StrToInt(tbProxyPort.Text);
  except
    TWXAuth.ProxyPort := 8080;
  end;
  ...
  TWXAuth.UserName := tbUser.Text;
  TWXAuth.UserReg := tbRegKey.Text;
...
procedure TfrmSetup.btnUpgradeClick(Sender: TObject);
begin
  (Owner as TModGUI).ShowForm(gfUpgrade);
end;
...
procedure TfrmSetup.cbUseAuthProxyClick(Sender: TObject);
begin
  tbProxyAddress.Enabled := cbUseAuthProxy.Checked;
  tbProxyPort.Enabled := cbUseAuthProxy.Checked;

  // only set to authenticate if not changed during form init.
  if (Assigned(tbProxyAddress.OnChange)) then
    FAuthenticate := TRUE;
end;

procedure TfrmSetup.ApplyNewReg(const NewUser, NewReg: string);
begin
  tbUser.Text := NewUser;
