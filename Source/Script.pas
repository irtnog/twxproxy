...
type
  ...
  private
    ...
    function ReadCode(var CodeRef : Pointer; ReadSize : Byte) : Pointer;
    function ReadIndexValues(var CodeRef : Pointer) : TStringArray;
...
begin
  ...
  TWXAuth.AuthKey := ScriptRef.FindCmd('SYS_NOP');
...
begin
  ...
  if (FWaitingForAuth) then
    TWXAuth.Subject.UnregisterObserver(Self);
...
procedure TScript.WaitForAuth;
begin
  FWaitingForAuth := True;
  TWXAuth.Subject.RegisterObserver(Self, True);
  TWXAuth.DoAuthenticate;
end;
...
function TScript.ReadIndexValues(var CodeRef : Pointer) : TStringArray;
var
  ParamType,
  IndexCount : Byte;
  I          : Integer;
begin
  ...
  IndexCount := Byte(ReadCode(CodeRef, 1)^);
  ...
  begin
    ...
    begin
      ParamType := Byte(ReadCode(CodeRef, 1)^);

      if (ParamType = PARAM_CONST) then
        // 32-bit constant reference
        Result[I] := Cmp.Params[Integer(ReadCode(CodeRef, 4)^)].Value
      ...
      begin
        ...
        Result[I] := Char(ReadCode(CodeRef, 1)^);
    end;
  end
  else
    Result := nil;
...
begin
  ConstRef := Word(ReadCode(CodeRef, 2)^);
  IndexValues := ReadIndexValues(CodeRef);
...
begin
  ...
  VarRef := Integer(ReadCode(CodeRef, 4)^);
  IndexValues := ReadIndexValues(CodeRef);

  if (Length(IndexValues) = 0) then
...
var
  ...
  begin
    ...
    CmdParams[Length(CmdParams) - 1] := TempParam;
  end;
  procedure AppendValue(CmdParam : TCmdParam);
  begin
    SetLength(CmdParams, Length(CmdParams) + 1);
    CmdParams[Length(CmdParams) - 1] := CmdParam;
  end;
begin
  ...
  try
    while (TRUE) do
    begin
      ParamType := Byte(ReadCode(cmdValues, 1)^);
      ...
      begin
        ...
        begin
          // 32-bit constant reference
          CodeInt := Integer(ReadCode(CmdValues, 4)^);
        ...
        begin
          ...
          CodeByte := Byte(ReadCode(CmdValues, 1)^);
    ...
    // free up temporary TConstParams
    if (Length(CmdParams) > 0) then
      for I := 0 to Length(CmdParams) - 1 do
        if (TCmdParam(CmdParams[I]).IsTemporary) then
...
begin
  ...
  try
    ...
    repeat
      ...
      // fetch script ID
      ExecScriptID := Byte(CodePos^);
      CodePos := Pointer(Integer(CodePos) + 1);

      // fetch line number
      Line := Word(CodePos^);
      CodePos := Pointer(Integer(CodePos) + 2);
  ...
  if (CmdAction = caAuth) and not TWXAuth.Authenticate then
    CmdAction := caStop;

  if (CmdAction = caAuth) then
  begin
    // jump back to re-execute the same command after authentication
    CodePos := LastCmd;
    WaitForAuth;
  end
  else if (CmdAction = caStop) then
  begin
    SelfTerminate;
    Result := TRUE;
  end;
