...
type
  ...
  private
    ...
    procedure miShowAnomoly(ClientIndex : Byte);
...
begin
  ...
  begin
    if (S.Anomoly) then
      TWXServer.Broadcast(ANSI_5 + 'Density ' + ANSI_14 + ': ' + ANSI_11 + Segment(S.Density) + ANSI_9 + ' (Anomoly)' + endl)
    else
...
begin
  ...
  begin
    ...
    AddItem(Menu);
    Menu := TTWXMenuItem.Create(Self, 'TWX_SHOWANOM', miShowAnomoly, 'Show all sectors with anomoly', '', 'A');
    Menu.Help := 'This option will show all the sectors that have been picked up with an anomoly by density scans.';
    AddItem(Menu);
...
procedure TModMenu.miShowAnomoly(ClientIndex : Byte);
var
  I : Integer;
  S : TSector;
begin
  TWXServer.Broadcast(endl + MENU_LIGHT + 'Showing all sectors with anomoly' + endl);

  for I := 1 to TWXDatabase.DBHeader.Sectors do
  begin
    S := TWXDatabase.LoadSector(I);

    if (S.Anomoly) and (S.Explored <> etNo) then
      DisplaySector(S, I);
  end;
